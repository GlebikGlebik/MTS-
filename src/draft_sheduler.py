...
class Sheduler:
    ...
    def tester(self):
        for host, host_resources in self.current_input['hosts'].items():
            used_cpu, used_ram = 0, 0
            for vm in self.current_output['allocations'].get(host, []):
                used_cpu += self.current_input['virtual_machines'][vm]['cpu']
                used_ram += self.current_input['virtual_machines'][vm]['ram']
        ...
        print(json.dumps(self.test_output, ensure_ascii=False, indent=2))
